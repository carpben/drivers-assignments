...
export const getShadow = (elevation: number = 3, opacityRatio: number = 1 / 2) => {
	if (elevation === 0) {
		return ""
	}
	...
	const getSize = (v1: number, v24: number) => `${v1 + Math.round(((v24 - v1) / 23) * extraEl)}px`

	const getShade = (y: number[], blur: number[], spread: number[], alpha: number) =>
